...
class PembelianController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($validate->fails()) {
            return redirect()->back()->withErrors('error', 'Pembelian gagal! Silakan periksa inputan Anda.');
        }
        ...
        try {
            ...
            DB::rollback();
            return back()->withErrors('error', 'Pembelian gagal! Terjadi kesalahan.');
        }
    }
    ...
    public function riwayat()
    {
        ...
        return view('pembelian.riwayat', compact('transactions', 'detailTransactions'));
    }

    public function deleteRiwayat($id)
    {

        $query = Transaksi::where('user_id', Auth::user()->id)
            ->where('id', $id)
            ->delete();

        if (is_iterable($query)) {
            foreach ($query as $data) {
                $query1 = TransaksiDetail::where('transaksi_id', $data)
                    ->delete();
            }
