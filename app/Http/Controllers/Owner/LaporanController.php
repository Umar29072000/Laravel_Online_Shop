...
class LaporanController extends Controller
{
    ...
    public function print(Request $request)
    {
        ...
        // Query to retrieve data for the report
        $query = DB::table('transaksi_details as a')
            ->join('transaksis as b', 'a.transaksi_id', '=', 'b.id')
            ->join('barangs as c', 'a.barang_id', '=', 'c.id')
            ->select(
            ...
            ->whereMonth('b.tanggal_transaksi', $bulan)
            ->whereYear('b.tanggal_transaksi', $tahun)
            ->groupBy('c.nama_barang', 'detail_transaksi_id', 'detail_barang_id', 'jumlah_beli', 'tanggal_transaksi', 'total_bayar', 'status', 'harga')
            ->orderBy('detail_transaksi_id', 'ASC')
            ->get();
        ...
        $pdf->loadView('owner.laporan.cetak', $data);
        // Generate PDF content
        $pdfContent = $pdf->output();

        // Save the PDF to a file
        file_put_contents('laporan_'.$bulan.'_'.$tahun.'.pdf', $pdfContent);
